		...
		while (true) {
			...
			if 		(sel == 1) { // 계좌 생성
				...
				// 해당되는 인덱스 넣기
					...
					int delidx = -1;
					int tempAccNumber = userManager.user[identifier].accCount;
					for (int i = 0; i < tempAccNumber; i++) {
					...
					// 해당되는 유저 계좌 카운트 감소
						...
						userManager.user[identifier].acc = new Account[tempAccNumber - 1];
						
						for (int i = 0; i < delidx; i++) {
							userManager.user[identifier].acc[i] = tempAcc[i];
						}
						for (int i = delidx; i < tempAccNumber - 1; i++) {
